...
class ResForgotpassword extends ChangeNotifier {
  ...
  Future<int> enterOtp() async {
    ...
    try {
      final response =
          await http.post(Uri.parse(Api.resforogt), body: {'identifier': email});
  ...
  Future<int> confirmOtp() async {
    ...
    try {
      final response = await http.post(Uri.parse(Api.resenterotp + uId), body: {
           "enteredOtp": otp
      }
      );
...
try{
    final response=await http.patch(Uri.parse(Api.ressavepassword),body: {
    "identifier":email,
